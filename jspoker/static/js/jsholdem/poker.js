...
function initialize_game () {
  console.log("Initialize game has been called");
  gui_hide_poker_table();
  gui_hide_dealer_button();
  gui_hide_fold_call_click();
...
function new_round () {
  console.log("NEW Round has been called");
  RUN_EM = 0;
  NUM_ROUNDS++;
  // Clear buttons
  gui_hide_fold_call_click();
...
function collect_cards () {
  console.log("Collect cards has been called");
  board = new Array(6);
...
function blinds_and_deal () {
  console.log("Calling blinds and deal function");
  SMALL_BLIND = 5;
  BIG_BLIND = 10;
  var num_playing = number_of_active_players();
  if (num_playing == 3) {
...
function deal_and_write_a () {
  console.log("Calling Dealing and writing A")
  var current_player;
  var start_player;
...
function delay_for_main () {
  console.log("Calling delay_for_main")
...
function main () {
  gui_hide_guick_raise();
  console.log("main is calling now");
  console.log("current betting amount is "+current_bet_amount);
  console.log("player[0] is "+players[0]);
  console.log("current_bettor_index is "+current_bettor_index);
    ...
    if (current_bettor_index == 0) {
      ...
      if (to_call == 0) {
        ...
        that_is_not_the_key_you_are_looking_for = function (key) {
          if (key == 67) {         // Check
            human_call();
            console.log("Pressing check button")
          } else {
        ...
        that_is_not_the_key_you_are_looking_for = function (key) {
          if (key == 67) {         // Call
            ...
            human_fold();
            console.log("Pressing Fold button")
          } else {
            return true;           // Not my business
          }
          return false;
...
function the_bet_function (player_index, bet_amount) {
  console.log("Calling the bet function now with player index as "+player_index+" and bet_amount as "+bet_amount);
  if (players[player_index].status == "FOLD") {
    return 0;
    console.log("Player index "+player_index+" is having player status of FOLD, so skipping the_bet_function");
    // FOLD ;
    ...
    console.log("Bet function is being executed with check or call");
    players[player_index].status = "CALL";
...
function human_call () {
  // Clear buttons
  console.log("Inside the human call function")
  gui_hide_fold_call_click();
...
function bet_from_bot (x) {
  console.log("Betting from bot "+x);
...
function get_num_betting () {
  ...
  console.log("Number of people who are not folded or busted and has money are "+n);
  return n;
}
